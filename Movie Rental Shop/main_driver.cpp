...
int main(){
	...
	rentalObject[total_records].setRental(tempRental_id,&customerObject,&movieObject,&dateObjectPur,&dateObjectRet,total,0);
	total_records++;
}
...
void discard_line(ifstream &FileName){
	...
	*/
	...
	void Print(Rental rentobj[], int size)
	{
		...
		for (int i=0; i <size; i++)
		{
			cout<<"\t"<<rentobj[i].getID()<<"\t\t	"<<rentobj[i].getCustomer().getCID()<<"\t\t"<<rentobj[i].getMovieCost() <<endl;
